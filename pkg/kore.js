...
// src/apps/kore/shaders/fragment-shader.glsl
var fragment_shader_default = "#include <commonDefs>\n#include <functions>\n#include <colourTable>\n#include <computeNormal>\n#include <simplex2D>\n#include <simplex3D>\n#include <rotate2D>\n\nin vec2 uv;\n\nlayout(std140) uniform ConfigData {\n  vec4 backgroundColour;\n  vec4 sandColour;\n  vec4 dataVector0;\n  vec4 dataVector1;\n  vec4 dataVector2;\n};\n\nuniform sampler2D wholeTexture; // A texture with random numbers for the whole screen\nuniform float baseUnit;\n\n#define sand sandColour.rgb\n#define background backgroundColour.rgb\n#define amplitude dataVector0.x\n#define threshold dataVector0.y\n#define variance dataVector0.z\n#define nScale dataVector0.w\n\n#define lightX dataVector1.x\n#define lightY dataVector1.y\n#define lightZ dataVector1.z\n#define speed dataVector1.w\n\n#define focus dataVector2.xy\n\n#define sandLow max(sandColour.rgb - vec3(.1), vec3(0.))\n#define backgroundLow max(backgroundColour.rgb - vec3(.1), vec3(0.))\n\nfloat random(vec2 st) {\n  return texture(wholeTexture, st).r;\n}\n\n#if defined(SINEWAVE)\n#define isAnimated true\nfloat computePosition(vec2 centre, vec2 coords) {\n  float d = length(centre - coords);\n\n  float p = 6./baseUnit * d + mod(-speed * time, 2. * PI);\n  float damper = gain(0.8, smoothstep(baseUnit * 0.4, baseUnit * 1.2, d));\n\n  float s = damper * amplitude * sin(p);\n  return s;\n}\n#elif defined(GAUSSIAN)\n#define isAnimated false\nfloat computePosition(vec2 centre, vec2 coords) {\n  float d = length(centre - coords);\n  float a = 10.; // amplitude\n  float b = 300.; // bell curve centre\n  float c = 80.; // variance\n  float s = gaussian(d, a, b, c);\n  float r = 900.;\n  float g = gaussian(d, a, r, c);\n  return (s + g) / 2.;\n}\n#elif defined(BREAKINGWAVE)\n#define isAnimated true\nfloat computePosition(vec2 centre, vec2 coords) {\n  float a = -0.0009;\n  float b = 0.0;\n  float c = 0.0;\n\n  float t = -2.1;\n\n  centre = 0.8 * rotate2D(t, centre);\n  coords = 0.8 * rotate2D(t, coords);\n  float d = abs(coords.x - centre.x)/(screen.x/5.);\n\n  vec2 transformedCoords = coords - centre;\n\n  // Compute the value of the quadratic equation at coords.x\n  float quadraticY = a * pow(transformedCoords.x, 2.) + b * transformedCoords.x + c;\n\n  // Calculate the distance from the point to the curve on the Y axis\n  float distance = abs(transformedCoords.y - quadraticY);\n  float heightScale = baseUnit / 2.;\n  float z = saturate((heightScale - distance) / heightScale);\n  z = gain(0.8, z);\n  z *= smoothstep(0., 1., clamp(1. - d, 0., 1.));\n  //z = 2. * smoothstep(0.0, 1.0, z);\n  return z;\n}\n#elif defined(PLATFORM)\n#define isAnimated false\nfloat computePosition(vec2 centre, vec2 coords) {\n  float d = length(centre - coords) + clamp(0.1 * time, 0., 60.);\n  return 10. * (3. * bias(.2, smoothstep(0., 700., d)) - bias(.2, smoothstep(500., 800., d)));\n}\n#endif\n\nfloat computeWedge(vec2 v, float dist, vec2 dir, float thetaMin, float thetaMax, vec2 start, vec2 end) {\n  float angle = smoothstep(thetaMin, thetaMax, dot(dir, v));\n  return angle * (smoothstep(start.x, start.y, dist) - smoothstep(end.x, end.y, dist));\n}\n\nvec3 wedgeColour(vec2 centre, vec2 fragCoord, out float h) {\n  vec2 v = fragCoord - centre;\n  float d = length(v);\n  v = v / d;\n\n  float angle = computeWedge(v, d, vec2(cos(.1*time), sin(.1*time)), .4, 1., vec2(400., 600.), vec2(800., 1000.));\n  vec3 colour = mix(sand, colourTable[3], angle);\n  h = angle;\n  angle = computeWedge(v, d, vec2(cos(.2*time), sin(.2*time)), .2, .8, vec2(300., 400.), vec2(500., 800.));\n  h = max(h, angle);\n  colour =  mix(colour, colourTable[0], angle);\n  angle = computeWedge(v, d, vec2(cos(-.3*time), sin(-.3*time)), .6, 1., vec2(500., 600.), vec2(600., 1200.));\n  h = max(h, angle);\n  colour = mix(colour, colourTable[4], angle);\n  return colour;\n}\n\nvec3 colourNoise(vec2 uv, vec3 baseColour, int id, out float h) {\n  h = snoise(vec3(vec2(AR, 1.) * uv * 0.5, 0.1*time)) * .5 + .5;\n  return mix(baseColour, colourTable[id], saturate(bias(.005, h)));\n}\n\nfloat noise(vec2 uv) {\n  return saturate(snoise(vec2(AR, 1.) * uv + 0.5 * time) * .5 + .5);\n}\n\nvec3 computeSandColour(vec2 uv, vec3 N, float h, vec3 colour, float r) {\n  vec3 L = normalize(vec3(lightX, lightY, lightZ));\n  float ld = saturate(dot(N, L));\n  float r1 = fract(random(uv) + 0.001 * baseUnit * (1. - ld));\n\n  float lightStrength = bias(variance, ld);\n  float grainStrength = bias(lightStrength, r1) + 1e-1;\n\n  vec3 colA = mix(colour, background, saturate(bias(threshold, grainStrength)));\n  colA = mix(colA, mix(colA, colour, step(.85, r1)), bias(0.1, r));\n  return colA;\n}\n\nvoid renderImage(inout vec4 fragColour, vec2 fragCoord) {\n  vec2 centre = screen / 2.;\n  vec2 invRes = 1. / screen;\n\n  float h = computePosition(centre - focus, fragCoord);\n  vec3 N = computeNormal(h, nScale);\n\n  vec2 uv = fragCoord * invRes;\n\n  float r, s;\n  vec3 c = colourNoise(uv, sand, 1, r);\n  c = colourNoise(.23*uv.yx, c, 6, s);\n  s = max(r, s);\n  c = colourNoise(rotate2D(0.13, uv*1.28), c, 3, r);\n  s = max(r, s);\n  c = colourNoise(rotate2D(-0.73, uv*.3), c, 4, r);\n  s = max(r, s);\n\n  vec3 colour = computeSandColour(uv, N, h * .5 + .5, c, s);\n  fragColour = vec4(colour, 1.0);\n}\n";
...
var ARRAY = {
  ...
  fromHexString(string2, target, rgbScale = 1) {
    const int = INT.fromHexString(string2);
...
var UniformBuffer = class {
  ...
  update(data2) {
    const gl = this.gl;
    gl.bindBuffer(gl.UNIFORM_BUFFER, this.resource);
    gl.bufferSubData(gl.UNIFORM_BUFFER, 0, data2, 0);
  }
  bindBuffer(unit) {
    this.gl.bindBuffer(this.gl.UNIFORM_BUFFER, this.resource);
...
function convertColor(input, outputFormat) {
  ...
  switch (outputFormat) {
    ...
    case "rgb_array" /* RGB_ARRAY */:
      return [r, g, b];
    case "rgb_normalized" /* RGB_NORMALIZED */:
...
function compileProgram(gl, vertexShader, fragmentShader, deleteShaders = true, shaders) {
  const progDef = [
...
var Mat4 = class _Mat4 extends Float32Array {
  ...
  static makeOrtho(left, right, bottom, top2, near, far) {
    const lr = 1 / (left - right);
    const bt = 1 / (bottom - top2);
    const nf = 1 / (near - far);
  ...
  static makeRotation(axis, theta) {
    ...
    const [cosTheta, sinTheta, t2] = [Math.cos(theta), Math.sin(theta), 1 - Math.cos(theta)];
    const [tx, ty, tz] = [t2 * axis[0], t2 * axis[1], t2 * axis[2]];
...
function contains(name2, map) {
  return map[name2] !== void 0;
}
...
function buildRenderContext(gl, prog) {
  ...
  for (let i = 0; i !== uniformCount; ++i) {
    ...
    if (info == null)
      continue;
    const { name: name2, type, size } = info;
    context[name2] = {
      value: createBaseType(type),
      location: gl.getUniformLocation(prog, name2),
      dataType: type,
      count: size
    };
  }
...
function buildUniformBlocks(gl, program) {
  ...
  for (let i = 0; i < numUniformBlocks; ++i) {
    const name2 = gl.getActiveUniformBlockName(program, i);
    if (!name2)
      continue;
    uniformBlocks[name2] = gl.getUniformBlockIndex(program, name2);
  }
  return uniformBlocks;
}
var EnhancedUniformMap = class _EnhancedUniformMap {
  map = {};
  ...
  set(name2, value) {
    if (!contains(name2, this.map)) {
      ...
      this.map[name2].value = value;
    }
  }
  has(name2) {
    return name2 in this.map;
  }
  delete(name2) {
    return delete this.map[name2];
  }
  clear() {
    this.map = {};
  }
  ...
  bindContext(gl) {
    for (const [name2, uniform] of this.entries()) {
      if (uniform.location === -1) {
        console.warn("Uniform location not found:", name2);
        continue;
      }
      if (!bindUniform(gl, uniform)) {
        console.warn("Failed to bind uniform:", name2);
      }
    }
  }
  static fromProgram(gl, prog) {
    const context = buildRenderContext(gl, prog);
    return new _EnhancedUniformMap(context);
  }
};
...
var ShaderLibrary = class {
  ...
  constructor() {
    ...
    this.includeTable.set("functions", functions);
  ...
  replaceInclude(source, include) {
    const replacement = this.includeTable.get(include.name);
  ...
  addTemplate(name2, sources) {
    this.templateTable.set(name2, sources);
  }
  hasTemplate(name2) {
    return this.templateTable.has(name2);
  }
  addInclude(name2, source) {
    this.includeTable.set(name2, source);
  }
  addDefine(name2, source) {
    this.defineTable.set(name2, source);
  }
...
`;
      ...
      if (definitions) {
        for (const [name2, value] of definitions) {
          curr += `#define ${name2} ${value}
`;
        }
      }
      curr += "\n";
...
// src/lib/Zed/lezer-glsl/glsl-highlight.js
var glslHighlighting = styleTags({
  "if else switch for while do case default return break continue discard": tags.controlKeyword,
  "struct precision layout subroutine": tags.definitionKeyword,
  "uniform attribute in out inout const flat invariant precise shared": tags.modifier,
  struct: tags.definitionKeyword,
  TypeQualifier: tags.modifier,
  StorageQualifier: tags.modifier,
  LayoutQualifier: tags.modifier,
  PrecisionQualifier: tags.unit,
  InterpolationQualifier: tags.modifier,
  InvariantQualifier: tags.modifier,
  "true false": tags.bool,
  TypeSpecifier: tags.typeName,
  "CallExpression/Identifier": tags.function(tags.variableName),
  StatementIdentifier: tags.labelName,
  "Declaration/FunctionPrototype/Identifier": tags.function(
    tags.definition(tags.variableName)
  ),
  "FunctionPrototype/Identifier": tags.function(tags.variableName),
  ...
  "FieldIdentifier/Swizzle": tags.string,
  "FieldIdentifier/Field": tags.special(tags.propertyName),
  Identifier: tags.literal,
  PreProcArg: tags.meta,
  "PreprocDirectiveName #ifdef #ifndef #if #define #else #endif #elif": tags.processingInstruction,
  "( )": tags.paren,
  "[ ]": tags.squareBracket,
  "{ }": tags.brace,
  ", ;": tags.separator
...
var setPref = (key, value) => {
  const prefs = JSON.parse(localStorage.getItem("prefs")) || defaultPrefs;
  prefs[key] = value;
  localStorage.setItem("prefs", JSON.stringify(prefs));
};

// src/lib/Zed/editor.ts
var editorConfig = {
  setShader: () => "",
  getShader: () => "",
  resize: () => {
...
void main() {
  renderImage(fragColour, gl_FragCoord.xy);
}
`;
var insertShader = (fragmentShader, plug) => fragmentShader.replace("##SHADER##", plug);
var codemirror = null;
var currentError = null;
var shift2 = fragmentShdr.split("##SHADER##")[0].split("\n").length;
...
var compilerLinter = linter((view) => {
  ...
  if (currentError) {
    ...
    while (parsed = errorRE.exec(currentError)) {
      ...
      let from, to;
      try {
        from = doc2.line(+line + 1 - shift2).from + +column;
        to = doc2.line(+line + 2 - shift2).from - 1;
      } catch (e) {
        from = 0;
        to = doc2.toString().length;
      }
      diagnostics.push({
        from,
        to,
...
var onChange = EditorView.updateListener.of((v) => {
  if (v.docChanged && !v.transactions.some((t2) => t2.isUserEvent("url"))) {
  }
});
var closeOnEscape = keymap.of([
  {
    key: "Escape",
    run: () => {
...
var compileOnSave = keymap.of([
  {
    key: "Mod-s",
    run: (editor) => {
      currentError = editorConfig.setShader(
        insertShader(fragmentShdr, editor.state.doc.toString()),
...
var initCodeMirror = () => {
  ...
  ensureSyntaxTree(codemirror.state, codemirror.state.doc.length, 1e3);
  codemirror.dispatch({});
  ...
  tree.iterate({
    enter: (node) => {
      if (node.name === "FunctionDefinition") {
        const nameNode = node.node.getChild("FunctionPrototype")?.node.getChild("Identifier");
        if (!nameNode) {
          return;
        }
        const name2 = codemirror?.state.doc.sliceString(
          nameNode.from,
          nameNode.to
        );
        if (!["renderImage"].includes(name2 ?? "")) {
...
var initEdit = (conf) => {
  ...
  toggler.addEventListener("click", () => {
    ...
    const [cls] = Object.entries(layouts).find(
...
var setEditSource = (source) => {
  if (codemirror) {
    ...
    editorConfig.setShader(insertShader(fragmentShdr, source), source);
  }
};

// src/lib/Zed/zed-editor.ts
var injectHTML = `
<aside class="commands">
    <button id="edit" class="button">\u270F\uFE0F</button>
    <button id="playpause" class="button">\u25B6\uFE0F</button>
    <button id="layout" class="button" style="display: none">\u{1F5A5}\uFE0F</button>
    <button id="importExport" class="button">\u{1F4E4}</button>
    <button id="status" class="button">\u{1F7E2}</button>
    <div id="fps" title="fps"></div>
</aside>
<canvas id="canvas"></canvas>
<aside id="editor" class="editor"></aside>

<div id="importExportPanel" class="panel">
    <div class="panel-content">
        <h2>Import/Export Shader</h2>
        <input type="file" id="importFile" accept=".glsl,.txt,.json">
        <button id="importButton" class="panel-button">Import Shader</button>
        <button id="exportButton" class="panel-button">Export Shader</button>
        <button id="closePanel" class="panel-button">Close</button>
    </div>
</div>
`;
var injectCanvas = `
...
void renderImage(inout vec4 colour, vec2 fragCoord) {
  vec2 uv = fragCoord / screen;
  colour = vec4(uv, 1. - uv.x, 1.);
}
...
var ZedEditor = class {
  ...
  statusButton = null;
  dataBuffer = new Uint8Array(2 * 4 * 4);
  resolution = new Float32Array(this.dataBuffer.buffer, 0, 4);
  timeData = new Float32Array(this.dataBuffer.buffer, 16, 4);
  shaders = [];
  onUpdate = null;
  onResize = null;
  onPaint = null;
  // The data we need for binding externally defined shaders
  hidden;
  constructor(containerId, hideEditor = false) {
    this.hidden = hideEditor;
    this.injectHTML(containerId);
    if (!this.hidden)
      this.injectFontLinks();
    this.canvasEl = document.getElementById("canvas");
    this.gl = this.canvasEl.getContext("webgl2");
    this.shaderLibrary = new ShaderLibrary();
    if (!this.hidden) {
      this.statusButton = document.getElementById("status");
      if (!this.statusButton)
        throw new Error("Status button not found");
      this.setShaderStatus();
    }
    this.prog = this.buildProgram(fragmentShdr2);
    if (this.prog == null)
      return;
    this.gl.useProgram(this.prog);
    this.ctx = EnhancedUniformMap.fromProgram(this.gl, this.prog);
    this.ctx.bindContext(this.gl);
    this.uniBlocks = buildUniformBlocks(this.gl, this.prog);
    this.gl.uniformBlockBinding(this.prog, this.uniBlocks.ShaderData, 0);
    if (this.onUpdate)
      this.onUpdate("", "");
    this.shaderBlock = new UniformBuffer(this.gl);
    this.shaderBlock.initialise(this.dataBuffer.byteLength);
    this.shaderBlock.bindBuffer(0);
    this.shaderBlock.update(this.dataBuffer);
  ...
  injectHTML(containerId) {
    ...
    container.innerHTML = this.hidden ? injectCanvas : injectHTML;
    const style = document.createElement("style");
    style.textContent = injectStyle;
    document.head.appendChild(style);
    this.editorElement = container;
  }
  setShaderStatus(errorMessage) {
    if (this.hidden || !this.statusButton)
      return;
    if (errorMessage) {
      this.statusButton.textContent = "\u{1F534}";
      this.statusButton.title = errorMessage;
      this.statusButton.style.cursor = "help";
    } else {
      this.statusButton.textContent = "\u{1F7E2}";
      this.statusButton.title = "Shader compiled successfully";
      this.statusButton.style.cursor = "default";
    }
  }
  injectFontLinks() {
    const links = [
    ...
    links.forEach((linkData) => {
      ...
      Object.entries(linkData).forEach(([attr, value]) => {
  ...
  buildProgram(source) {
    const precision = /* @__PURE__ */ new Map();
    precision.set("float", "highp");
    precision.set("int", "highp");
    if (this.shaders[1]) {
      this.gl.deleteShader(this.shaders[1]);
      this.shaders[1] = null;
    }
    if (this.prog) {
      this.gl.deleteProgram(this.prog);
    }
    const template = this.shaderLibrary.buildShader([source], this.defines, "300 es", precision);
    const vertex = this.shaders[0] !== void 0 ? this.shaders[0] : vertexShdr;
    const prog = compileProgram(this.gl, vertex, template[0], false, this.shaders);
    if (typeof prog === "string") {
      this.setShaderStatus(prog);
      console.error(prog);
      return null;
    }
    this.setShaderStatus();
  ...
  setShader(source, shaderName) {
    if (shaderName)
      this.currentShaderName = shaderName;
    if (this.hidden) {
      const shdr = insertShader(fragmentShdr, source);
      this.buildProgram(shdr);
      this.update(shdr, source);
    } else {
      setEditSource(source);
    }
  }
  rebuild() {
    this.setShader(this.currentSource);
  }
  bindUniformBlock(name2, index) {
    if (this.prog == null)
      return;
    const block = this.uniBlocks[name2];
    if (block === void 0)
      return;
    this.gl.uniformBlockBinding(this.prog, block, index);
  }
  update(source, content2) {
    const prog = this.buildProgram(source);
    if (prog == null)
      return "Failed to compile shader";
    this.currentSource = content2;
    this.saveShader();
    this.gl.useProgram(prog);
    this.ctx = EnhancedUniformMap.fromProgram(this.gl, prog);
    this.ctx.bindContext(this.gl);
    this.uniBlocks = buildUniformBlocks(this.gl, prog);
    this.gl.uniformBlockBinding(prog, this.uniBlocks.ShaderData, 0);
    if (this.onUpdate)
      this.onUpdate(source, content2);
  ...
  draw(time, dt) {
    ...
    this.gl.useProgram(this.prog);
    if (this.onPaint)
      this.onPaint(time, dt);
    this.ctx.bindContext(this.gl);
    this.timeData.set([time, dt, 0, 0]);
    this.shaderBlock?.update(this.dataBuffer);
  ...
  resize() {
    ...
    let rW = Math.round(dPR * w);
    let rH = Math.round(dPR * h);
  ...
  saveShader() {
    const shaderContent = this.shader;
    if (this.currentShaderName) {
      this.saveToLocalStorage(this.currentShaderName, shaderContent);
    } else {
      this.promptShaderName(shaderContent);
    }
  }
  promptShaderName(shaderContent) {
    const namePrompt = document.createElement("div");
    namePrompt.innerHTML = `
      <div class="panel" style="display: block;">
        <div class="panel-content">
          <h2>Name Your Shader</h2>
          <input type="text" id="shaderNameInput" placeholder="Enter shader name">
          <button id="saveShaderButton" class="panel-button">Save</button>
          <button id="cancelSaveButton" class="panel-button">Cancel</button>
        </div>
      </div>
    `;
    document.body.appendChild(namePrompt);
    const saveButton = namePrompt.querySelector("#saveShaderButton");
    const cancelButton = namePrompt.querySelector("#cancelSaveButton");
    const input = namePrompt.querySelector("#shaderNameInput");
    saveButton?.addEventListener("click", () => {
      const name2 = input.value.trim();
      if (name2) {
        this.currentShaderName = name2;
        this.saveToLocalStorage(name2, shaderContent);
  ...
  saveToLocalStorage(name2, content2) {
    try {
      localStorage.setItem(`shader_${name2}`, content2);
    } catch (error) {
      console.error("Error saving shader to localStorage:", error);
    }
  }
  loadShader(name2) {
    const shaderContent = localStorage.getItem(`shader_${name2}`);
    if (shaderContent) {
      this.setShader(shaderContent, name2);
      return true;
    } else {
      console.error("Shader not found in localStorage");
      return false;
    }
  }
  setupEventListeners() {
    let raf;
    let pause = !this.hidden;
    const playpause = document.getElementById("playpause");
    if (pause && !playpause)
      throw new Error("Missing play/pause element");
    let prev = performance.now() * 1e-3;
    let curr = performance.now() * 1e-3;
    let dt = 0;
    let time = 0;
    const paint = () => {
      prev = curr;
      curr = performance.now() * 1e-3;
      dt = curr - prev;
      time += dt;
      this.draw(time, dt);
    ...
    if (pause && playpause) {
      ...
      if (!importButton || !exportButton || !importExportButton || !importExportPanel || !closePanel || !importFile) {
        throw new Error("Missing import/export elements");
      }
      ...
      exportButton.addEventListener("click", () => {
        const shaderContent = this.shader;
        const jsonContent = JSON.stringify({ shader: shaderContent });
    ...
    this.resize();
    const callbacks = {
      getShader: () => shaderExample,
      setShader: (shader, content2) => {
        shaderExample = shader;
        return this.update(shader, content2);
      },
      resize: () => {
        this.resize();
      }
    ...
    initEdit(callbacks);
    setEditSource(shaderExample);
    if (!pause)
      paint();
  }
};
var shaderExample = `void renderImage(inout vec4 fragColour, vec2 fragCoord) {
  vec2 uv = fragCoord / screen;
  fragColour = vec4(uv, 1. - uv.x, 1.);
}
`;

// src/apps/kore/kore.ts
var guiContainer = document.getElementById("gui-container");
var gui = new lil_gui_esm_default({ closeFolders: true, container: guiContainer });
var colourTable = `
...
var controllerData = {
  ...
  lightZ: 1,
  speed: 0.9,
  waveFunction: "SineWave" /* SineWave */
};
var data = gui.addFolder("Shader");
data.addColor(controllerData, "background");
data.addColor(controllerData, "sand");
data.add(controllerData, "intensity", 0, 10);
data.add(controllerData, "threshold", 0, 1);
data.add(controllerData, "variance", 0, 1);
data.add(controllerData, "normalScale", 0, 2, 0.01);
data.add(controllerData, "lightX", -1, 1);
data.add(controllerData, "lightY", -1, 1);
data.add(controllerData, "lightZ", -1, 1);
var app = gui.addFolder("Application");
app.add(controllerData, "speed", 0, 2, 0.01);
var dataBuffer = new Uint8Array(5 * 4 * 4);
var background2 = new Float32Array(dataBuffer.buffer, 0, 4);
var sand = new Float32Array(dataBuffer.buffer, 4 * 4, 4);
var dataVector0 = new Float32Array(dataBuffer.buffer, 2 * 4 * 4, 4);
var dataVector1 = new Float32Array(dataBuffer.buffer, 3 * 4 * 4, 4);
var dataVector2 = new Float32Array(dataBuffer.buffer, 4 * 4 * 4, 4);
var Renderer = class {
  zed;
  canvas;
  gl;
  ...
  baseUnit = 0;
  constructor() {
    this.zed = new ZedEditor("zed-container", true);
    this.canvas = this.zed.canvas;
    this.gl = this.zed.context;
    this.shaderLibrary = this.zed.library;
    this.shaderLibrary.addInclude("colourTable", colourTable);
    this.zed.defineMap.set("SINEWAVE", "");
    this.shaderBlock = new UniformBuffer(this.gl);
    this.shaderBlock.initialise(dataBuffer.byteLength);
    this.shaderBlock.bindBuffer(1);
    this.updateBuffer();
    this.zed.onUpdateCallback = (source, content2) => {
      this.zed.bindUniformBlock("ConfigData", 1);
      this.zed.uniforms.set("wholeTexture", 2);
      this.zed.uniforms.set("baseUnit", this.baseUnit);
      console.log("baseUnit:", this.baseUnit);
    };
    this.zed.onResizeCallback = (w, h) => {
      this.screen.set(w, h);
      dataVector2.set([w / 2, -h / 2, 0, 0]);
      this.generateWholeTexture(w, h, window.devicePixelRatio);
      const baseUnit = w > h ? h : w;
      this.baseUnit = baseUnit / 5;
      this.zed.uniforms.set("baseUnit", this.baseUnit);
      console.log("baseUnit:", this.baseUnit);
      this.updateBuffer();
    };
    this.zed.onPaintCallback = (t2, dt) => {
      this.onPaint(t2, dt);
    };
    this.generateWholeTexture(this.screen.x, this.screen.y, window.devicePixelRatio);
    if (!this.zed.loadShader("kore")) {
      console.log("could not find shader");
      this.zed.setShader(fragment_shader_default, "kore");
    }
    this.zed.resize();
    this.updateBuffer();
    this.zed.repaint();
    ...
    app.add(controllerData, "waveFunction", {
      sineWave: "SineWave" /* SineWave */,
      gaussian: "Gaussian" /* Gaussian */,
      breakingWave: "BreakingWave" /* BreakingWave */,
      platform: "Platform" /* Platform */
    }).onChange(() => {
      this.zed.defineMap.delete("SINEWAVE");
      this.zed.defineMap.delete("GAUSSIAN");
      this.zed.defineMap.delete("BREAKINGWAVE");
      this.zed.defineMap.delete("PLATFORM");
      this.zed.defineMap.set(controllerData.waveFunction.toUpperCase(), "");
      this.zed.rebuild();
    });
  }
  onPaint(time, dt) {
    let x = 0;
    let y = 0;
    if (controllerData.waveFunction === "BreakingWave" /* BreakingWave */) {
      if (Math.abs(dataVector2[0]) < 10 || Math.abs(dataVector2[1]) < 10) {
        x = this.screen.x * 0.5;
        y = -this.screen.y * 0.5;
      } else {
        x = dataVector2[0];
    ...
    dataVector2.set([x, y, 0]);
    this.shaderBlock?.update(dataBuffer);
  }
  updateBuffer() {
    const dPR = window.devicePixelRatio ?? 1;
    if (controllerData.waveFunction === "BreakingWave" /* BreakingWave */) {
      dataVector0.set([3.85 * dPR, 0.521, 0.521, 15 * 0.18 / this.baseUnit]);
      dataVector1.set([-0.426, 0.656, 1, controllerData.speed]);
    } else {
      dataVector0.set([controllerData.intensity * dPR, controllerData.threshold, controllerData.variance, 15 * controllerData.normalScale / this.baseUnit]);
      dataVector1.set([controllerData.lightX, controllerData.lightY, controllerData.lightZ, controllerData.speed]);
    }
    sand.set(convertColor(controllerData.sand, "rgb_normalized" /* RGB_NORMALIZED */));
    background2.set(convertColor(controllerData.background, "rgb_normalized" /* RGB_NORMALIZED */));
    this.shaderBlock?.update(dataBuffer);
    this.zed.repaint();
  }
  generateWholeTexture(w, h, dPR) {
    w = Math.round(w / dPR);
    h = Math.round(h / dPR);
    ...
    const gl = this.gl;
    this.wholeTexture = this.wholeTexture !== null ? this.wholeTexture : gl.createTexture();
    gl.activeTexture(gl.TEXTURE2);
    gl.bindTexture(gl.TEXTURE_2D, this.wholeTexture);
