...
var RenderApp = class {
  ...
  rebuild(replayIntro = false) {
    ...
    console.log("uniqueRadii:", uniqueRadii);
    const pointsMap = /* @__PURE__ */ new Map();
    for (const radius of uniqueRadii) {
      const data = makePoissonSphere(this.gl, radius);
      if (data)
        pointsMap.set(radius, data);
      else {
        console.error("Failed to generate points with radius:", radius);
        return;
      }
    }
  ...
  initiatePresetChange(target, name) {
    if (this.sourceName === name)
      return;
    this.targetName = name;
    this.controlTarget = target;
  ...
  computeCurrPreset(dt2) {
    ...
    if (this.controlTime > 1) {
      ...
      for (const key in this.controlSource) {
        if (key !== "layers" && typeof this.controlSource[key] === "number") {
          controls[key] = lerp(this.controlSource[key], this.controlTarget[key], this.controlTime);
        }
      }
      return controls;
    }
  ...
  /*
    public initiateLerp (target: any): void {
      ...
      if (this.controlSource.layers !== this.controlTarget.layers) {
        ...
        this.controls.layers = this.controlTarget.layers
      }
    }
  ...
  render(dt2) {
    ...
    for (let p = 0; p < passes.length; ++p) {
      ...
      this.ctx[0].pointSize.value = controls.pointSize * factor;
      ...
      this.ctx[0].opacity.value = Math.min(controls.opacity, globalOpacity, passOpacity[p]);
    ...
    mutations.forEach(function(mutation) {
      if (mutation.type === "attributes" && mutation.attributeName === "class") {
        slides.forEach((slide, index) => {
          if (slide.classList.contains("swiper-slide-active") && performance.now() - lastTrigger > 100) {
            const n = names[index];
...
function setupTriggerAccordion(accordionName = accordions[1]) {
  ...
  const observer2 = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting && !state.isTriggered) {
        state.isTriggered = true;
        list[0]?.initiateLerp(JSON.parse(configurations.ai), "ai");
      } else {
        if (!entry.isIntersecting && state.isTriggered) {
...
var observer = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    mutation.addedNodes.forEach((node) => {
      if (checkCreate(node, "#canvas", 0) && observers.length === 0) {
        setupSlideTransitionObserver();
        setupTriggerAccordion();
      }
